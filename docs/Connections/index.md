...
- [**Upload to a server or drive**](#server-connections): Connect to a server or drive and upload files over the internet to back them up or so that others can access them. This feature is commonly used by organizations that want to centralize data or files collected by Tella users, or by individuals looking to back up their files.
