...
If Tella users download a template that has properties that are not supported in Tella, those properties won't be shown in the app. However, users will still be able to submit the entities, and admins will be able to access these entities, including all the properties that were filled, without any problems. However, if the entity cannot be submitted because there are required fields not visible, Tella will show an error and save the submission in the Outbox tab. Templates will need to be refreshed to submitted after an admin has made the required fixes in the template configuration.
</Admonition>
