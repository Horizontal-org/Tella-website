...
import {Note, Info} from '@site/src/components/Admonitions';
...
Tella collects information associated with the files you create through, or import into, the App. This may include, for example, file name, the folder where the file is stored in the App, date and time the file was created and last modified in the App, and hash value of the file. Collectively, this data is known as “File Metadata”.

Tella also collects information about your mobile device and internet connection, including device manufacturer, hardware, device ID (a unique identifier of the device provided by the operating system, which remains the same if the App is reinstalled), screen size, language, locale, connection status, network type, mobile data type, wireless network MAC address, and internet protocol (IP) addresses. Collectively, this data is known as “Device Metadata.”

Tella also collects information that helps identify the location and environment of the device when a file was captured. This includes location (including latitude, longitude, accuracy, altitude, and the time of the location reading), location provider, location speed, cell towers detected by your device, names of the Wi-Fi networks detected near your device, and, when available on the device, ambient light and temperature. Collectively, this data is known as “Location Metadata”.

File, device and location metadata are stored encrypted in the App on your device and are not sent to the Company. They are only shared with others if you choose to do so, for example by sharing or exporting a file together with its verification information, or by sending it to a Deploying Organization (see Information you send through the App).

File, device and location metadata are only collected if you opt-in by turning on Verification Mode or Preserve metadata when importing (EXIF metadata) in your App settings. If you do not want Tella to collect this information, do not activate Verification Mode nor Preserve metadata when importing.
...
- Metadata collection by enabling “Verification Mode” in your app settings. You can limit what is collected by not granting Tella access to your device's location, including approximate location (without it, location, cell tower and Wi-Fi information are not collected).
