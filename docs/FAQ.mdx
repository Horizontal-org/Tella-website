...
Tella is compatible with devices running Android 5.0 or later and iOS 15.0 or later.
...
\* Tella's [verification mode](/features#verification-mode) captures verification information (file metadata) that can be used to corroborate evidence, but it does not prove that a file is authentic or has not been altered.
