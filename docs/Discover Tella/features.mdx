...
import {Note, Tip, Warning, Danger, Info} from '@site/src/components/Admonitions';
...
* Save the [verification information](#saving-and-sharing-verification-information) of a file as a CSV file, stored in the same folder as the original file.
...
When activated, every time a user takes a photo or a video, or records audio, Tella automatically captures verification information (file metadata). This verification information can be used to corroborate evidence, cross-check with other facts known about the event or about the area where it was captured.

To turn it on, go to **Settings ⚙️ > General > Verification mode**. Tella will ask for permission to access the device's location. Verification information is only collected for files captured with Tella's camera and audio recorder while verification mode is on. It is not collected for files imported from the device.

<Warning>
Verification information can identify the user and their device (for example their location, IP addresses or device ID). Only share it if doing so will not put you or others at risk.
</Warning>

### Viewing verification information {/* #viewing-verification-information */}

To see the verification information of a file, open the file's options menu (⋮), either from a folder or while viewing the file, and select **Verification information**. While viewing the file, it can also be opened by tapping the verification information button (🛡️) in the top bar. This option is only available for files that have verification information.


### Saving and sharing verification information {/* #saving-and-sharing-verification-information */}

Verification information is stored as a CSV file, with one column per field. It can be opened with any spreadsheet app (like Excel, Google Sheets or LibreOffice). The CSV file has the same name as the original file (for example, `photo.jpg` → `photo.csv`).

- **Save it inside Tella**: on the Verification information screen, tap **Save as CSV**. The CSV is saved, encrypted, in the same folder as the original file, and can be attached to reports and connections like any other file. The CSV stays linked to the original file: if the original file is deleted, its CSV is deleted too.
- **Share it or export it to the device**: when sharing or exporting files that have verification information, Tella asks whether to include it. Choose **Share files + verification information** to include the CSV, or **Share files only** to leave it out. When exporting to the device with verification information, Tella also saves the CSV inside Tella, if it wasn't saved before.
...
|---|---|---|
| File name | The file's name in Tella. | Tella. |
| File path | The folder where the file is stored in Tella, for example `/Evidence/March`, or `/` if it is not in a folder. | Tella's folder structure. |
| File hash | A unique alphanumeric sequence (a SHA-256 hash) that identifies the file's exact content. | Calculated by Tella from the original, unencrypted file when it is saved. |
| File created | The date and time the file was saved in Tella. | Tella. |
| File modified | The date and time the file was last renamed or moved in Tella. | Tella. |

**Device** 📱

| Field | What it means | Where it comes from |
|---|---|---|
| Manufacturer | The name of the device manufacturer. | Android system information. |
| Hardware | The manufacturer and model of the device. | Android system information. |
| Device ID | A unique identifier of the device. It stays the same if Tella is deleted and reinstalled, and it is different for Tella and Tella FOSS on the same device. | Provided by Android (Android ID). |
| Screen size | The size of the screen's diagonal, in inches. | Calculated from the screen's resolution and pixel density, so it may differ slightly from the manufacturer's specification. |
| Wi-Fi MAC (WiFi MAC) | The hardware identifier of the device's Wi-Fi connector. | Android. On recent Android versions apps can no longer access this value, so this field can be empty. |
| Language | The language used by Tella: the language selected in Tella's settings or, if none was selected, the language of the device. | Tella and Android language settings. |
| Locale | The country or region the device is set to, as a country code. | Android region settings. |

**Network** 📶

Permissions: Cell info requires location permission (precise or approximate). Wi-Fi info requires precise location permission and location turned on. Without them, these fields are not available.

| Field | What it means | Where it comes from |
|---|---|---|
| Connection status (Network) | Whether the device had a network connection. | Android connectivity information. |
| Network type | Whether the device was connected through Wi-Fi or mobile data. | Android connectivity information. |
| Data type | If the device was connected through mobile data, the type of mobile network (for example, LTE or 3G). Empty when connected through Wi-Fi. | Android telephony information. |
| IPv4 / IPv6 | The IP addresses assigned to the device's network connections. These are usually local addresses (for example, assigned by the Wi-Fi router or a VPN), not the public IP address seen on the internet. | The device's network interfaces. |
| Cell info (Cells) | The identifiers (country code, network code and cell ID) of the cell towers detected by the device. | Android telephony information. |
| Wi-Fi info (WiFis) | The names of the Wi-Fi networks detected near the device. | A Wi-Fi scan made by Tella. |

**Location** 📍

Permissions: requires precise location permission and location turned on. Without them, these fields are not available.

| Field | What it means | Where it comes from |
|---|---|---|
| Location provider | How the location was obtained: GPS (satellites) or network (Wi-Fi and cell towers, usually less accurate). | Android location services. |
| Latitude / Longitude | The coordinates of the device, in degrees. | Android location services. |
| Altitude | The altitude of the device, in meters. | Android location services. |
| Accuracy (Location accuracy) | The estimated accuracy of the location, in meters. The smaller the number, the more precise the location. | Android location services. |
| Location speed | The speed at which the device was moving, in meters per second. | Android location services. |
| Timestamp of GPS reading (Location timestamp) | The date and time the location was measured. | Android location services. |

**Other** 🌡️

| Field | What it means | Where it comes from |
|---|---|---|
| Ambient temperature | The temperature around the device. | The device's temperature sensor. |
| Light | The level of light around the device, in lux. | The device's light sensor. |

<Info>
Some fields may not be available depending on the device, the permissions that you have chosen to give Tella, the device settings and the location circumstances. For example, the location may not be available indoors, and many devices don't have a temperature sensor.

On the Verification information screen, fields without a value show **Not Available**. In the CSV, they are left empty, and if there is no location, cell or Wi-Fi information, those columns are not included.
</Info>

<Info title="Contact us">
If you need help to better understand Verification Mode, feel free to [contact us](/contact-us) 😊
</Info>
...
If any of the selected files has [verification information](#verification-mode), Tella asks whether to include it as a CSV file or to share only the files.
...
The camera also offers the following options:
- **Flash**: choose between on, off, or auto.
- **Grid**: shows a grid overlay to help frame and align photos and videos.
- **Zoom**: pinch to zoom in or out before taking a photo or recording a video.
