...
10. **Nearby Sharing of files**: Tella allows establishing a secure connection with devices that are the same physical location and transfer files end-to-end encrypted, from one Tella vault to another one. [Nearby Sharing](/nearby-sharing) works fully offline, even on insecure or networks being surveilled, and it works cross-platform (between Android, iOS and Desktop).
...
* File management:
    ...
    * Files sent to a [server connection](/for-organizations) will be stored un-encrypted on the server, unless the server has implemented disk encryption. People who has access to the server might be able to see the files. Tella makes sure that the data travels encrypted to the server but cannot guarantee the security practices of the server. If you choose to connect to a Google Drive or Dropbox instance, or work with a third party organization for hosting your data, we recommend to investigate if the organization itself can access your data.
