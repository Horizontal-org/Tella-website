...
- Uwazi connection improvements:
    - Made sure that Primary Documents' field allow PDF selection from both Tella Vault or the device.
    - Allowed Supporting Documents to include all types of file selection from the Tella Vault or the device.
    - Made sure to update Draft, Outbox and New entity templates when pressing the "Refresh Templates" button.
- Migrated translation management to Weblate. 
- Bug fixes and minor improvements:
    - Fix crash during concurrent large video imports.
