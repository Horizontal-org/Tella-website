...
### Android: Tella 3.4.0 (257) - Released on October 5, 2026 {/* #android-tella-340257---released-on-oct-5-2026 */}
- Improvements to [Verification mode](/features#verification-mode):
    - Redesigned the Verification information screen: information is now organized in categories (File, Device, Network, Location and Other), each with a help screen explaining what every field means. 
    - Added the option to save verification information as a CSV file inside Tella so it can be attached to reports and connections like any other file. 
    - Tella now asks to enable GPS before recording videos and audio (not only before taking photos), and uses network-based location in addition to GPS, so a location can be found faster, especially indoors.
    - Tella no longer keeps waiting for Wi-Fi information when there are no Wi-Fi networks around or Wi-Fi is turned off (for example when offline or in airplane mode): the file is saved as soon as the location is found.
    - Reviewed the verification fields: added a "File created" field, and the Device ID now uses the identifier provided by Android, which stays the same if Tella is reinstalled. See the [full list of fields and what they mean](/features#what-information-is-collected).
- Improvements to Connections:
    - Made delete confirmations and server URL error messages consistent across all connections.
- Bug fixes and minor improvements:
    - Fixed a bug where choosing "Share files only" from the file viewer still included the verification information.
    - Fixed verification fields that were empty or showed wrong values, including the file path, the date the file was last modified, and Wi-Fi information missing from the CSV when no cell information was available.
    - Fixed several issues with verification mode prompts and messages, including when recording audio.
    - Text fields (for example when renaming files or creating folders) now get focus automatically and open the keyboard.
    - Removed an unnecessary lock timeout warning when taking a photo to send through Nearby Sharing. 
    - Fixed minor UI issues, including the "no internet" message in reports being hidden behind the keyboard and the color of the delete action in the files list.
- Updated translations. Please consider helping us [localize Tella](/translating-tella).



### iOS: Tella 3.3.0 (156) - Released on September 16, 2026 {/* #ios-tella-330156---released-on-sep-16-2026 */}
- Improvements to the Tella [Camera](/features#camera-photos-and-videos):
    - Added zoom when taking photos or recording videos.
    - Added automatic flash mode: the flash can now be set to on, off or auto.
- Added the possibility to zoom in on images and videos while previewing them.
- Added support for the Geolocation property in the [Uwazi](/uwazi) connection.
- Added a warning when users try to delete a file from their Tella vault that is pending submission (in Drafts or Outbox) in one of their connections.
- Loaded volunteer translations from Ukrainian and Arabic.
- Bug fixes and minor improvements:
    - Added a visual indicator (green check) when a file is successfully sent through Nearby Sharing.
    - Fixed a visual bug in the background activity tray on older iPhones.
    - Added a confirmation prompt when users try to go back while waiting to receive files in Nearby Sharing.
- Technical improvements:
    - Raised the minimum supported iOS version to [iOS 15](/faq#what-phones-support-tella), as required by Apple.



### Android: Tella FOSS 3.4.0 (257) (based on Android 3.4.0) - Released on October 5, 2026 {/* #android-tella-foss-340257-based-on-android-340---released-on-oct-5-2026 */}
- Improvements to [Verification mode](/features#verification-mode):
    - Redesigned the Verification information screen: information is now organized in categories (File, Device, Network, Location and Other), each with a help screen explaining what every field means. 
    - Added the option to save verification information as a CSV file inside Tella so it can be attached to reports and connections like any other file. 
    - Tella now asks to enable GPS before recording videos and audio (not only before taking photos), and uses network-based location in addition to GPS, so a location can be found faster, especially indoors.
...
#### Android: Tella 3.3.2 (254) - Released on August 27, 2026 {/* #android-tella-332254---released-on-aug-27-2026 */}
- Hotfix: Fixed "Move to folder" when the destination folder is empty, and prevented moving a folder into itself.

#### Android: Tella 3.3.1 (252) - Released on August 26, 2026 {/* #android-tella-331252---released-on-aug-26-2026 */}
- Hotfix: Fixed a bug that made files stored inside folders also appear at the top level of "All files" after updating, so they looked duplicated.

#### Android: Tella 3.3.0 (250) - Released on August 21, 2026 {/* #android-tella-330250---released-on-aug-21-2026 */}
- All translation management is now done on Weblate. Please consider contributing by helping us [localize Tella](/translating-tella).
- Updated the onboarding flow and the advanced configuration options, including camouflage.
- Multiple improvements to the Uwazi connection:
    - Implemented count indicators to show the number of entities in each tab (Drafts, Outbox and Submitted).
    - Reviewed and improved copy and wording.
    - Added the possibility to swipe between tabs.
    - Improved UX to make it more accessible, including updated fonts, colors and sizes.
    - Unified and reused existing components.
    - Added Server URL validation: Tella now shows an error message if the URL is invalid or does not correspond to a valid Uwazi instance.
    - Added a warning when users try to delete a media file from their Tella vault that is pending submission in one of their Uwazi entities.
    - Added confirmation snackbars for destructive actions.
- Bug fixes and minor improvements:
    - Fixed a crash and an incorrect error message ("problem creating folder") that appeared after sending the first report to a Google Drive Connection.


#### Android: Tella 3.2.0 (248) - Released on July 16, 2026 {/* #android-tella-320248---released-on-jul-16-2026 */}
...
#### iOS: Tella 3.2.0 (153) - Released on August 10, 2026 {/* #ios-tella-320153---released-on-aug-10-2026 */}
- Uwazi connection improvements:
    - Primary Documents' field now allow PDF selection from both the Tella Vault or the device.
    - Supporting Documents can now include all types of file selection from the Tella Vault or the device.
    - Draft, Outbox and New entity templates update when pressing the "Refresh Templates" button.
- All translation management is now done on Weblate. Please consider contributing by helping us [localize Tella](/translating-tella).
- Bug fixes and minor improvements:
    - Fixed a crash that happened during concurrent large video imports.
    - Refactored the format of all the app's strings to maximize translation memory usage.
    - Improved error messages for incorrect PIN in Nearby Sharing's manual connection.
    - Fixed "Welcome to Tella" screen that was shown briefly before the Lock screen.
    - UX fixes for smaller screens.
...
#### iOS: Tella 3.0.0 (144) - Released on May 20, 2026 {/* #ios-tella-300-144---released-on-may-20-2026 */}
- Implemented [Nearby Sharing](/nearby-sharing): Nearby Sharing lets users securely share files in physical proximity. Nearby Sharing is fully offline, available across platforms and devices, and assures end-to-end encryption. 
- Implemented security fixes detected by our most recent security audit (to be published soon!).
- New languages support: Ukrainian.
- Bug fixes and minor improvements:
...
#### Android: Tella FOSS 3.3.2 (254) (based on Android 3.3.2) - Released on August 28, 2026 {/* #android-tella-foss-332254-based-on-android-332---released-on-aug-28-2026 */}
- Hotfix: Fixed a bug that made files stored inside folders also appear at the top level of "All files" after updating, so they looked duplicated.
- Hotfix: Fixed "Move to folder" when the destination folder is empty, and prevented moving a folder into itself.
- Hotfix: Updated the French success message shown after submitting a form.

#### Android: Tella FOSS 3.3.0 (251) - Released on August 26, 2026 {/* #android-tella-foss-330251---released-on-august-26-2026 */}
- All translation management is now done on Weblate. Please consider contributing by helping us [localize Tella](/translating-tella).
- Updated the onboarding flow and the advanced configuration options, including camouflage.
- Multiple improvements to the Uwazi connection:
    - Implemented count indicators to show the number of entities in each tab (Drafts, Outbox and Submitted).
...
#### Android: Tella FOSS 3.2.0 (248) (based on Android 3.2.0) - Released on July 18, 2026 {/* #android-tella-foss-320248-based-on-android-320---released-on-jul-18-2026 */}
- Implemented security fixes detected by our most recent [security audit](/security-and-privacy#security-audits)
    - Implemented mTLS and enforced certificate pinning for both the sender and receiver in Nearby Sharing. This is a mitigation for man-in-the-middle attacks.
- Bug fixes and minor improvements:
    - Simplified the Nearby Sharing manual connection flow: pre-filled the port field and added automatic dot insertion after each octet while typing the IP address.
