...
2. **Criptografia em trânsito:** Todos os dados sendo transferidos entre o Tella e os servidores (veja a [seção de Conexões](/connections)) são criptografados por meio do protocolo [Segurança da Camada de Transporte (TLS)](https://en.wikipedia.org/wiki/Transport_Layer_Security). O Tella torna impossível enviar dados para o servidor sem a criptografia TLS.
