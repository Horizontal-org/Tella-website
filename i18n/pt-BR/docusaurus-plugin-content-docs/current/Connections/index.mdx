...
:::info All server connections are **offline first**, with built-in mechanisms to pause and resume uploads when the internet is unreliable. Learn more about [offline data collection here](/features#offline-data-collection). :::

:::danger Even though data is transferred to the server securely (SSL), files stored on the server are **not** encrypted. Anyone with access to the server can view them. :::

<Button label="Continue reading about Server Connections and how organizations use Tella" link="/for-organizations"/>


:::tip The [Share](/features#share-button) feature is also available for sharing files directly with other apps installed on the device. :::
